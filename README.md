...
| проверка | результат |
|---|---|
| cookies из профиля в Debian, клиенты `web`/`mweb`/`tv`/default | у всех **2 совпадения из 25** с браузером телефона |
...
**Выгружать их повторно не придётся.** Когда cookies стареют, приложение
продлевает их **той же сессией**: грузит `cookies.txt` в чистый контекст
Chromium (не в профиль Debian — иначе подмешается его личность и станция
уедет), заходит на YouTube, Google прокручивает токены, файл переписывается.
Личность сессии сохраняется, станция вместе с ней. Профиля того браузера для
этого не нужно — он и недоступен: чужой каталог приложения Android не читается
без root, а личность, как выяснилось, живёт в самих cookies.

Повторить выгрузку нужно только если сессия умерла совсем (сменил пароль,
вышел из аккаунта) — тогда ⚙ так и скажет.

Вход через `scripts/login.sh` снова делает хозяином профиль в Debian — метка
внешних cookies снимается автоматически.

### Если cookies из браузер-слоя: состав берётся из браузера

Если cookies свои (вход через `scripts/login.sh`, а не импорт), состав микса
приложение **спрашивает у браузер-слоя**: Chromium в Debian открывает ссылку,
снимает очередь из DOM и тут же экспортирует cookies той же сессии — список и
авторизация гарантированно из одного состояния ([`auth/mix.py`](auth/mix.py)).
Станция будет своя, но зато состав совпадёт с тем, что этот браузер показывает,
и будет воспроизводимым.

При импортированных cookies этот шаг **пропускается** — yt-dlp и так отдаёт
нужную станцию, и тратить минуту на proot незачем.

Цена: разбор микса занимает ~30–60 с (старт proot + загрузка страницы) вместо
пары секунд. Обычные плейлисты `PL…` это не затрагивает — они статические и
идут прежним быстрым путём. Выключить: `export TY_MIX_BROWSER=0`.
...
что телефон. Разное — cookies не подхватились (`bash scripts/login.sh`).
...
|---|---|
| В миксе не те треки, что в приложении YouTube | Состав микса берётся из браузер-слоя. Проверь ⚙: должно быть «качаю под аккаунтом». Если браузер-слой не поставлен, приложение откатывается на yt-dlp, а у того **своя станция** — RD-микс привязан к сессии. Тогда `bash scripts/setup-termux.sh`, либо вставь список видео вручную |
| Микс добавляется 30–60 секунд | Так и задумано: приложение поднимает Chromium в proot и снимает очередь из браузера. Плейлисты `PL…` идут быстрым путём. Не нужно — `export TY_MIX_BROWSER=0` |
