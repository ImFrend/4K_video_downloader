...
- ✅ Параллельные загрузки — 4 одновременно (`config.CONCURRENT_DOWNLOADS`), как в 4KVD.
...
Лёгкий локальный интерфейс (stdlib, **ноль зависимостей** — работает даже на
слабых/старых телефонах). Рисует браузер телефона; сервер слушает **строго
`127.0.0.1`** — 100% on-device, наружу не доступен.

```bash
python main.py web      # поднимет сервер и сам откроет браузер
```

Возможности: вставка плейлиста из буфера, очередь до 5 My Mix, выбор платформы/
качества, слайдер параллелизма с цветовым риском (🟢→🔴), живые прогресс-бары,
тап по миксу → детали с потреково. Загрузка переживает закрытие вкладки
(состояние держит сервер). Обновление cookies — авто, без кнопки.
