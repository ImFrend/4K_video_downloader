...
### My Mix: состав берётся из браузера

**RD-микс привязан к сессии браузера, а не к аккаунту.** Это проверено на
устройстве и оказалось решающим:

| проверка | результат |
|---|---|
| cookies валидны (`netscape_has_auth`) | да |
| клиенты плеера `web` / `mweb` / `tv` / по умолчанию | у всех **2 совпадения из 25** |
| повтор того же запроса | список **тот же** — выдача стабильна |

То есть у yt-dlp своя станция, у браузера своя, и обе устойчивы. Ни клиент, ни
авторизация на это не влияют — «правильного» списка по ссылке просто не
существует, есть список конкретной сессии.

Поэтому состав микса приложение **спрашивает у браузера**, который у него и так
есть: Chromium в Debian с сохранённым профилем открывает ссылку, снимает очередь
из DOM и тут же экспортирует cookies той же сессии — список и авторизация
гарантированно из одного состояния ([`auth/mix.py`](auth/mix.py)). От тебя —
обычная ссылка, больше ничего.
