...
#### Если консоль не даёт вставить

Chrome защищается от самовзлома и требует напечатать фразу «разрешить вставку»
руками — а без кириллической раскладки это тупик. Консоль тогда не нужна:
сделай **закладку** (в поле адреса закладки вставка не блокируется).

Правый клик по панели закладок → «Добавить страницу» → в поле URL вставь целиком:

```
javascript:(()=>{const r=[...document.querySelectorAll('ytd-playlist-panel-video-renderer a[id="wc-endpoint"]')].map(a=>{const u=new URL(a.href,location.origin);return{i:+(u.searchParams.get('index')||0),id:u.searchParams.get('v')}}).filter(x=>x.id).sort((a,b)=>a.i-b.i);const ids=[...new Set(r.map(x=>x.id))];if(!ids.length){alert('Очередь не найдена — открой микс так, чтобы справа был список');return}prompt('Треков: '+ids.length+'. Скопируй (Ctrl+C):',ids.join(','))})()
```

Назови «Снять очередь». Теперь: открой микс → нажми закладку → выскочит окошко с
готовым списком через запятую, жми Ctrl+C. Всё, консоль не понадобилась.

Селекторы тут без `#` (взяты как `[id="…"]`) намеренно: решётка в адресе закладки
обрезала бы код как якорь.
